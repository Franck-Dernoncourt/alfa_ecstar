...
def get_accuracy():
    accuracy = [0 for i in range(NB_SOLUTIONS_PER_LOG)]
    count = 0
    ...
    for solution in range(NB_SOLUTIONS_PER_LOG):
        ...
        for key, value in fitness_vs_packages_stats_per_solution[solution].items():
            ...
            for j in value:
                accuracy[solution] += j
        accuracy[solution]=accuracy[solution] / float(count)       
        
    print accuracy      
    return accuracy
        
def parse_dump_training_and_testing(dump_file):
    if not os.path.isfile(dump_file+  "test.log") or os.path.isfile(dump_file+  "train.log"):
        return -1        
    parse_dump(dump_file+  "test.log")
    parse_dump(dump_file+ "train.log")
...
#    file_output = open(dump_file + "_parsed.txt", 'w')
    ...
    for cur_line in file:
        ...
        if (cur_line.find('+++++') >= 0):
            solution = int(cur_line.split("_")[2]) - 1
            actions[solution] = []
