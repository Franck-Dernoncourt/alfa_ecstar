function [gp]=NeuroSkyconfig2(gp);
...
gp.runcontrol.num_gen=200;				        % Number of generations to run for including generation zero
...
% load cleaned files
data = csvread('all-logs.dat-clean.csv');
...
gp.userdata.xtrain=x(1:200,:); %training set (inputs)
gp.userdata.ytrain=y(1:200,1); %training set (output)
gp.userdata.xtest=x(201:250,:); %testing set (inputs)
gp.userdata.ytest=y(201:250,1); %testing set (output)
